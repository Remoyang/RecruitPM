...
def user_add():
    ...
    if not re.match(r'^[A-Za-z]{1}[A-Za-z0-9_-]{3,15}$', account):
        return jsonify(errno=RET.DATAERR, errmsg='账号格式错误')
    # 校验姓名
    ...
    if not type(role_id) == int:
        return jsonify(errno=RET.DATAERR, errmsg='角色格式错误')
    # 校验角色是否存在
    try:
        ...
        if not role:
            return jsonify(errno=RET.DATAEXIST, errmsg='角色不存在')
    # 校验用户名是否已注册
    try:
...
def role_add():
    ...
    # 校验角色名格式
    if not type(name) == str:
        return jsonify(errno=RET.DATAERR, errmsg='角色名格式错误')
    # 校验角色权限格式
    if not type(auths) == list:
        return jsonify(errno=RET.DATAERR, errmsg='角色权限格式错误')
    # 校验角色权限是否存在
    try:
    ...
    if set(auth_list) < set(auths):
        return jsonify(errno=RET.DATAERR, errmsg='角色权限错误')
    # 校验角色名是否已注册
    try:
        role = Role.query.filter_by(name=name).first()
    except Exception as e:
        current_app.logger.error(e)
        return jsonify(errno=RET.DBERR, errmsg='获取角色信息异常')
    else:
        if role:
            return jsonify(errno=RET.DATAEXIST, errmsg='角色名已注册')
    # 写入日志
...
def auth_add():
    ...
    # 校验权限名
    if not type(name) == str:
        return jsonify(errno=RET.DATAERR, errmsg='权限名格式错误')
    # 校验权限规则
    if not type(url) == str:
        return jsonify(errno=RET.DATAERR, errmsg='权限规则格式错误')
    # 校验权限名是否已注册
    try:
        ...
        if auth:
            return jsonify(errno=RET.DATAEXIST, errmsg='权限名已注册')
    ...
    try:
        ...
        # 如果存储数据发生异常，需要进行回滚操作
        db.session.rollback()
        return jsonify(errno=RET.DBERR, errmsg='保存权限信息异常')
...
def auth_edit():
    ...
    db.session.add(role)
    db.session.commit()
    return jsonify(errno=RET.PARAMERR, errmsg='权限编辑成功')
    pass
